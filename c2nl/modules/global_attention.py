    ...
    def forward(self, source, memory_bank, memory_lengths=None,
        ...
        if self._coverage:
            ...
            if one_step:
                ...
                multiplier = multiplier.unsqueeze(
                    0).expand(batch, *multiplier.size())
                multiplier = multiplier.cuda() if align.is_cuda else multiplier

                # B x tlen-1 x slen
                penalty = torch.bmm(multiplier, exp_score[:, :-1, :])
                no_penalty = torch.ones_like(penalty[:, -1, :])  # B x slen
                penalty = torch.cat([no_penalty.unsqueeze(
                    1), penalty], dim=1)  # B x tlen x slen
