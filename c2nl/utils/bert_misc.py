...
def tens2sen(t, tokenizer=None, src_vocabs=None, extra_vocabs=None, first_offset=1e5):
    sentences = []
    ...
    for idx, s in enumerate(t):
        ...
        for wt in s:
            ...
            if word in [tokenizer.bos_token_id]:
                continue
            if word in [tokenizer.eos_token_id]:
                break
            if tokenizer and word < len(tokenizer):
                sentence += [tokenizer.convert_ids_to_tokens(word)]
            elif src_vocabs and (len(tokenizer) <= word < len(tokenizer)+first_offset):
                word = word - len(tokenizer)
                sentence += [src_vocabs[idx][word]]
            elif extra_vocabs and word >= len(tokenizer)+first_offset:
                word = word - len(tokenizer) - first_offset
                sentence += [extra_vocabs[idx][word]]
            else:
                sentence += [str(word)]
        ...
        if len(sentence) == 0:
            ...
            sentence = [str(tokenizer.pad_token)]
