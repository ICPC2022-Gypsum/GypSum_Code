...
class Vocabulary(object):
    def __init__(self, tokenizer, no_special_token=False):
        self.tokenizer = tokenizer

        if no_special_token:
            self.tok2ind = {tokenizer.pad_token: tokenizer.pad_token_id,
                            tokenizer.unk_token: tokenizer.unk_token_id}
            self.ind2tok = {tokenizer.pad_token_id: tokenizer.pad_token,
    ...
    def __getitem__(self, key):
        if type(key) == int:
            return self.ind2tok.get(key, self.tokenizer.unk_token)
        elif type(key) == str:
            return self.tok2ind.get(self.normalize(key), self.tok2ind.get(self.tokenizer.unk_token))
        else:
    ...
    def tokens(self):
        ...
        tokens = [k for k in self.tok2ind.keys()
                  if k not in {self.tokenizer.pad_token, self.tokenizer.unk_token}]
        return tokens
